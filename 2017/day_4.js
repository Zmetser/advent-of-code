...
function isUnique(phrase, passphrases) {
  return !passphrases.find(_ => _ === phrase);
}

function hasOnlyUniqueWords(word, words) {
  if (words.length === 0) {
    return false;
  }

  return isUnique(words.pop(), words) ? hasOnlyUniqueWords(words[0], words) : true;
}

function hasAnagrams(words) {
  const wordsWithLettersInOrder = words.map(word => word.split('').sort().join(''));
  return hasOnlyUniqueWords(wordsWithLettersInOrder[0], wordsWithLettersInOrder);
}

const validWithOnlyUniqueWords = lines.filter(passphrase => {
  const words = passphrase.split(' ');
  return !hasOnlyUniqueWords(words[0], words);
});

const validWithoutAnagrams = lines.filter(passphrase => {
  return !hasAnagrams(passphrase.split(' '));
});

solution(validWithOnlyUniqueWords.length);
solution(validWithoutAnagrams.length);